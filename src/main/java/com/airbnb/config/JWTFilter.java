       ...
       String token=request.getHeader("Authorization");
        if(token != null && token.startsWith("Bearer "))
        {
          ...
          //  System.err.println(username);
            ...
            if(opUser.isPresent()){
                AppUser appUser = opUser.get();//we get exact user object
                UsernamePasswordAuthenticationToken
                        ...
                        auth.setDetails( new WebAuthenticationDetails(request));//as we request job we never get job
                SecurityContextHolder.getContext().setAuthentication(auth);//sgs
            }
        }
